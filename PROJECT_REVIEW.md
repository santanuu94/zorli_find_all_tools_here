...
- Secrets (GitHub repo → Settings → Secrets and variables → Actions; **never in code/logs/git**):
  ...
  - Install history: an earlier lockfile committed from Windows failed strict `npm ci` sync on Linux ("'npm ci' can only install packages when your package.json and package-lock.json ... are in sync. Missing: @emnapi/core / @emnapi/runtime from lock file"). Root cause was stale `dev`/`optional` flags on the `@emnapi/wasi-threads 1.2.3` + `tslib 2.8.1` wasm32-wasi subtrees. Fixed by regenerating `package-lock.json` (verified via `npm ci --dry-run` + `npm run lint` exit 0); all pipelines are back on `npm ci`.
