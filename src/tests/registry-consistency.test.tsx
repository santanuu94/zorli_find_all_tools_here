...
describe('Deep-link routing', () => {
  ...
  test('renders the tool catalogue when the URL is /tools', () => {
    window.history.pushState({}, '', '/tools');
    render(<App />);

    expect(screen.getByRole('heading', { level: 1, name: /Explore All\s+Tools/i })).toBeInTheDocument();
    expect(screen.getByText('Image Compressor')).toBeInTheDocument();
  });

  test('deep link to a coming-soon tool shows the honest placeholder, not a working tool', () => {
    window.history.pushState({}, '', '/tools/image/image-compressor');
    render(<App />);

    expect(screen.getByRole('heading', { level: 1, name: 'Image Compressor' })).toBeInTheDocument();
  ...
  test('an unknown tool route falls through to the 404 page', () => {
